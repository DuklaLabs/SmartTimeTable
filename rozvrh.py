...
# Projdi každého učitele
    ...
    for obj in data[Json_file.replace('.json','')]:
        ...
        slozka = f"{Json_file.replace('.json','')}/{name}"
...
cesta_k_souboru = os.path.join(slozka_scriptu, nazev_souboru)

get_links(url_s_parametrem, cesta_k_souboru)

# vytvoření souboru teachers.json
with open('teachers.json', 'w', encoding='utf-8') as soubor:
    soubor.write(str(teachers).replace('\'','"'))

# vytvoření souboru classes.json
with open('classes.json', 'w', encoding='utf-8') as soubor:
    soubor.write(str(classes).replace('\'','"'))

# vytvoření souboru rooms.json
with open('rooms.json', 'w', encoding='utf-8') as soubor:
    soubor.write(str(rooms).replace('\'','"'))
    
with open('conf.json', 'w', encoding='utf-8') as soubor:
